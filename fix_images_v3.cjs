...
const provincesContent = fs.readFileSync('src/data/provinces.ts', 'utf-8');

// Parse the content to find all food and attraction names
const provinceDataRegex = /"([^"]+)":\s*{[\s\S]*?}/g;
let match;
let updatedContent = provincesContent;

// First, update province main images
for (const [name, url] of Object.entries(provinceImages)) {
  const pRegex = new RegExp(`"${name}":\\s*{[\\s\\S]*?"image":\\s*"[^"]+"`, 'g');
  updatedContent = updatedContent.replace(pRegex, (m) => {
    return m.replace(/"image":\s*"[^"]+"/, `"image": "${url}"`);
  });
}

// Then, update all food items and attractions with keyword-based loremflickr
// This ensures they are UNIQUE and RELEVANT to their names
const foodAttrRegex = /"name":\s*"([^"]+)",\s*"image":\s*"[^"]+"/g;
updatedContent = updatedContent.replace(foodAttrRegex, (match, name) => {
  // Clean name for URL
...
// Special case for attractions that might have image on next line or slightly different formatting
const attrDeepRegex = /"name":\s*"([^"]+)",[\s\S]*?"image":\s*"[^"]+"/g;
updatedContent = updatedContent.replace(attrDeepRegex, (match, name) => {
    if (match.includes('"lat":')) { // It's an attraction
        const keyword = name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, ',');
...
fs.writeFileSync('src/data/provinces.ts', updatedContent);
console.log("Updated provinces.ts with dynamic keyword-based images from LoremFlickr.");
